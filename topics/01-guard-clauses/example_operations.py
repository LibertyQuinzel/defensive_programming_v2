...
def add(a: Number, b: Number) -> float:
    ...
    Rationale: explicit checks make debugging easier and prevent downstream
    surprises if callers pass unexpected types.
    """
    if not _is_number(a):
...
def subtract(a: Number, b: Number) -> float:
    ...
    Explanation:
    - Same validation rules as `add` and `multiply`.
    - Raises `InvalidOperandError` when inputs are invalid.
    - Returns the numeric difference as a float.
    """
    if not _is_number(a):
...
def multiply(a: Number, b: Number) -> float:
    ...
    Explanation: see `add` for rationale. Multiplication follows the
    same validation rules and returns a float product.
    """
    if not _is_number(a):
...
def divide(a: Number, b: Number) -> float:
    ...
    Explanation:
    - Validates both operands using `_is_number`.
    - Explicitly checks divide-by-zero and raises `DivisionByZeroError` with a
      helpful message (preferred over letting a raw ZeroDivisionError bubble up
      unmodified).
    - Returns the quotient as a float.

    Error modes:
    - `InvalidOperandError` when either operand is invalid
    - `DivisionByZeroError` when `b` is zero
    """
    if not _is_number(a):
